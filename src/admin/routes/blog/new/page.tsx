...
const NewBlogPostPage = () => {
  ...
  async function handleSave() {
    ...
    if (!form.excerpt) { alert("Excerpt is required"); return }
    if (!content) { alert("Article content is required"); return }
  ...
  return (
    <div className="flex flex-col gap-4 p-6 max-w-4xl mx-auto">
      ...
      <div className="flex items-center justify-between">
        <Heading level="h1">New Blog Post</Heading>
          ...
          <div>
            ...
            <p className="text-sm text-ui-fg-subtle mt-1">
              {form.status === "published"
                ? "Will be visible on live site"
                : "Saved as draft — hidden from site"}
            </p>
          </div>
          <select
            value={form.status}
